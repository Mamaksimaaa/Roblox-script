...
function MinecraftLib:CreateWindow(title, config)
    ...
    -- horizontal tab bar height and search bar height (always active)
    local HTAB_H = mobile and 34 or 30
    local SEARCH_H = mobile and 30 or 26
    local _hTabMode = false  -- current state, toggled via settings
    ...
    -- ---- horizontal tab bar (always created, toggled via settings) ----
    local HTabBar = Create("Frame", {
        Name = "HTabBar",
        Size = UDim2.new(1, 0, 0, HTAB_H),
        Position = UDim2.new(0, 0, 0, TITLE_H),
        BackgroundColor3 = self._theme.SecondaryBG,
        BorderSizePixel = 0, ZIndex = 2, Visible = false, Parent = Main,
    })
    PixelBevel(HTabBar, self._theme, 2)
    local HTabList = Create("ScrollingFrame", {
        Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, BorderSizePixel = 0,
        ScrollBarThickness = 3, ScrollBarImageColor3 = self._theme.Scrollbar,
        CanvasSize = UDim2.new(0, 0, 0, 0), AutomaticCanvasSize = Enum.AutomaticSize.X,
        ScrollingDirection = Enum.ScrollingDirection.X, ZIndex = 3, Parent = HTabBar,
    })
    Create("UIListLayout", {
        FillDirection = Enum.FillDirection.Horizontal,
        Padding = UDim.new(0, 3), SortOrder = Enum.SortOrder.LayoutOrder, Parent = HTabList,
    })
    AddPadding(HTabList, 3, 3, 5, 5)
    self._hTabList = HTabList

    -- ---- search bar (always created, shown in both modes) ----
    local _searchOffset = 0  -- extra Y offset when search is visible
    local SearchBar = Create("Frame", {
        Name = "SearchBar",
        Size = UDim2.new(1, -TAB_W - 10, 0, SEARCH_H),
        Position = UDim2.new(0, TAB_W + 5, 0, TITLE_H + 2),
        BackgroundColor3 = self._theme.SecondaryBG,
        BackgroundTransparency = 0.15, BorderSizePixel = 0, ZIndex = 10, Visible = true, Parent = Main,
    })
    PixelBevel(SearchBar, self._theme, 1)
    local SearchIcon = Create("TextLabel", {
        Size = UDim2.new(0, SEARCH_H, 1, 0), BackgroundTransparency = 1,
        Text = "🔍", TextSize = mobile and 11 or 12, Font = FONT,
        TextColor3 = self._theme.TextSecondary, ZIndex = 11, Parent = SearchBar,
    })
    local SearchBox = Create("TextBox", {
        Name = "SearchInput",
        Size = UDim2.new(1, -SEARCH_H - 4, 1, -4), Position = UDim2.new(0, SEARCH_H, 0, 2),
        ...
        ClearTextOnFocus = false, BorderSizePixel = 0, ZIndex = 11, Parent = SearchBar,
    })
    PixelText(SearchBox)
    ...
    -- ---- layout applier ----
    local function _ApplyTabLayout()
        if _hTabMode then
            -- horizontal: hide vertical bar, show horizontal bar
            TabBar.Visible = false
            Divider.Visible = false
            HTabBar.Visible = true
            local topOff = TITLE_H + HTAB_H
            SearchBar.Size = UDim2.new(1, -10, 0, SEARCH_H)
            SearchBar.Position = UDim2.new(0, 5, 0, topOff + 2)
            ContentArea.Size = UDim2.new(1, -6, 1, -(topOff + SEARCH_H + 8))
            ContentArea.Position = UDim2.new(0, 3, 0, topOff + SEARCH_H + 4)
        else
            -- vertical: show vertical bar, hide horizontal bar
            TabBar.Visible = true
            Divider.Visible = true
            HTabBar.Visible = false
            SearchBar.Size = UDim2.new(1, -TAB_W - 10, 0, SEARCH_H)
            SearchBar.Position = UDim2.new(0, TAB_W + 5, 0, TITLE_H + 2)
            ContentArea.Size = UDim2.new(1, -TAB_W - 6, 1, -(TITLE_H + SEARCH_H + 8))
            ContentArea.Position = UDim2.new(0, TAB_W + 3, 0, TITLE_H + SEARCH_H + 4)
        end
    end
    _ApplyTabLayout()

    -- ---- search filtering logic ----
    local function _FilterSearch(query)
        query = query:lower():gsub("^%s*(.-)%s*$", "%1")
        for _, tabInfo in ipairs(self._tabs) do
            local scroll = tabInfo.content
            for _, child in ipairs(scroll:GetChildren()) do
                if child:IsA("Frame") or child:IsA("TextButton") then
                    local lbl = child:FindFirstChildWhichIsA("TextLabel", true)
                    local visible = true
                    if query ~= "" then
                        local text = lbl and lbl.Text:lower() or child.Name:lower()
                        visible = text:find(query, 1, true) ~= nil
                    end
    ...
    SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
        _FilterSearch(SearchBox.Text)
    end)
    SearchBox.Focused:Connect(function() Tween(SearchBar, {BackgroundColor3 = self._theme.TertiaryBG}, 0.15) end)
    SearchBox.FocusLost:Connect(function() Tween(SearchBar, {BackgroundColor3 = self._theme.SecondaryBG}, 0.15) end)
    ...
    function self:SetTheme(name)
        ...
        HTabBar.BackgroundColor3 = t.SecondaryBG
        SearchBar.BackgroundColor3 = t.SecondaryBG
        ...
        for _, tabInfo in ipairs(self._tabs) do
            ...
            if tabInfo.hbtn then
                tabInfo.hbtn.BackgroundColor3 = isActive and t.TabActive or t.TabInactive
                tabInfo.hbtn.TextColor3 = isActive and t.TextPrimary or t.TextSecondary
                tabInfo.hActiveBar.BackgroundColor3 = t.Accent
            end
        end
    ...
    function self:IsHorizontalTabs() return _hTabMode end

    -- Set a custom background image URL (downloads & applies at runtime)
    function self:SetCustomBackground(url)
        ...
        task.spawn(function()
            ...
            if ok and type(data) == "string" and #data > 100 then
                EnsureFolder(FOLDER)
                local path = FOLDER .. "/custom_bg.png"
                pcall(writefile, path, data)
                if HasAsset then
                    local ok2, id = pcall(getcustomasset, path)
                    if ok2 then MainBg.Image = id end
                end
            else
                warn("[MinecraftLib] Custom background download failed: " .. tostring(url))
            end
        end)
    end
    ...
    function self:AddTab(name)
        ...
        local TabBtn = Create("TextButton", {
            ...
            TextColor3 = t.TextSecondary, TextSize = FONT_BODY, Font = FONT, BorderSizePixel = 0, LayoutOrder = tabOrder,
        ...
        -- horizontal tab button (compact pill style)
        local htabW = math.max(60, #name * (FONT_BODY - 1) + 20)
        local HTabBtn = Create("TextButton", {
            Name = "HTab_" .. name, Size = UDim2.new(0, htabW, 1, -6), BackgroundColor3 = t.TabInactive, Text = name,
            TextColor3 = t.TextSecondary, TextSize = FONT_BODY - 1, Font = FONT, BorderSizePixel = 0, LayoutOrder = tabOrder,
            TextTruncate = Enum.TextTruncate.AtEnd, ZIndex = 4, Parent = self._hTabList,
        })
        PixelText(HTabBtn); PixelBevel(HTabBtn, t, 1); CreateInsetShadow(HTabBtn, t, 2)
        local HActiveBar = Create("Frame", {Size = UDim2.new(1, 0, 0, 3), Position = UDim2.new(0, 0, 1, -3), BackgroundColor3 = t.Accent, BorderSizePixel = 0, Visible = false, ZIndex = 6, Parent = HTabBtn})
        ...
        HTabBtn.Activated:Connect(SelectThis)
        -- hover effects for vertical
        ...
        HTabBtn.MouseEnter:Connect(function()
            if self._activeTab ~= tabInfo then Tween(HTabBtn, {BackgroundColor3 = self._theme.TertiaryBG}, 0.15); HTabBtn.TextColor3 = self._theme.TextHover end
        end)
        HTabBtn.MouseLeave:Connect(function()
            if self._activeTab ~= tabInfo then Tween(HTabBtn, {BackgroundColor3 = self._theme.TabInactive}, 0.15); HTabBtn.TextColor3 = self._theme.TextSecondary end
        end)
    ...
    function self:AddSettingsTab(name)
        ...
        -- Theme dropdown (built-in themes)
        ...
        -- Custom background URL (creates "Custom" theme entry on apply)
        local bgUrlBox = ui:AddTextbox({Name = "Custom background URL", Placeholder = "https://i.imgur.com/...", Flag = "_CustomBG",
            Tooltip = "Введи ссылку на изображение и нажми Enter для применения фона"})
        bgUrlBox.Box.FocusLost:Connect(function(enter)
            if enter then
                local url = bgUrlBox:Get()
                if url ~= "" then
                    self:SetCustomBackground(url)
                    self:Notify({Title = "Theme", Content = "Custom background applied!", Type = "Success"})
                end
            end
        end)

        -- Custom theme: accent color hex
        local accentBox = ui:AddTextbox({Name = "Accent color (hex)", Placeholder = "#50C82D", Flag = "_AccentHex",
            Tooltip = "Hex-цвет акцента (#RRGGBB). Нажми Enter для применения"})
        accentBox.Box.FocusLost:Connect(function(enter)
            if enter then
                local hex = accentBox:Get()
                local c = FromHex(hex)
                if c then
                    local t2 = self._theme
                    t2.Accent = c
                    t2.AccentHover = Color3.new(math.min(c.R * 1.2, 1), math.min(c.G * 1.2, 1), math.min(c.B * 1.2, 1))
                    t2.AccentPress = Color3.new(c.R * 0.8, c.G * 0.8, c.B * 0.8)
                    t2.SliderFill = c; t2.ToggleOn = c; t2.NotifAccent = c
                    for _, refresh in ipairs(self._themedRefreshers) do pcall(refresh, t2) end
                    self:Notify({Title = "Theme", Content = "Accent color updated!", Type = "Success"})
                else
                    self:Notify({Title = "Theme", Content = "Invalid hex color", Type = "Error"})
                end
            end
        end)
        ...
        -- ---- Layout section ----
        local layout = tab:AddSection("Layout")

        -- Horizontal tabs toggle
        layout:AddToggle({Name = "Horizontal tabs", Default = _hTabMode, Flag = "_HTabMode",
            Tooltip = "Переключить вкладки в горизонтальный режим (сверху)",
            Callback = function(v) self:SetTabMode(v) end})

        -- Window Size presets
        layout:AddDropdown({Name = "Window size preset", Options = {"Small (480×360)", "Default (600×440)", "Large (750×520)", "Wide (820×420)"},
            Default = "Default (600×440)", Tooltip = "Быстро задать размер окна",
            Callback = function(v)
                local presets = {
                    ["Small (480×360)"]  = {480, 360},
                    ["Default (600×440)"] = {600, 440},
                    ["Large (750×520)"]  = {750, 520},
                    ["Wide (820×420)"]   = {820, 420},
                }
                local p = presets[v]
                if p then
                    Tween(Main, {Size = UDim2.new(0, p[1], 0, p[2])}, 0.25, Enum.EasingStyle.Back)
                end
            end})
