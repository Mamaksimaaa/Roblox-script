...
function MinecraftLib:CreateWindow(title, config)
            ...
            self._window:_track(UserInputService.InputBegan:Connect(function(inp, gp)
                ...
                if open and inp.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    if pos.X < ap.X or pos.X > ap.X + as.X or pos.Y < ap.Y or pos.Y > ap.Y + as.Y then
                        if not (pos.X >= row.AbsolutePosition.X and pos.X <= row.AbsolutePosition.X + row.AbsoluteSize.X and pos.Y >= row.AbsolutePosition.Y and pos.Y <= row.AbsolutePosition.Y + row.AbsoluteSize.Y) then
                            open = false
                            ListFrame.Visible = false
    ...
    --// Author Tab (built-in)
    local AuthorTab = self:AddTab("Author")

    local AuthorContainer = Create("Frame", {
        Size = UDim2.new(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
        BackgroundTransparency = 1,
        LayoutOrder = 0,
        ZIndex = 4,
        Parent = AuthorTab._parent,
    })
    Create("UIListLayout", {
    ...
    AddPadding(AuthorContainer, 8, 8, 8, 8)

    -- Logo
    local LogoRow = Create("Frame", {
        Size = UDim2.new(1, 0, 0, self._mobile and 120 or 140),
        BackgroundTransparency = 1,
        LayoutOrder = 1,
        ZIndex = 4,
        Parent = AuthorContainer,
    ...
    Create("ImageLabel", {
        Name = "AuthorLogo",
        Size = UDim2.new(0, self._mobile and 100 or 120, 0, self._mobile and 100 or 120),
        Position = UDim2.new(0.5, self._mobile and -50 or -60, 0, 5),
        BackgroundTransparency = 1,
        Image = "https://i.ibb.co/dw1k7FPp/e32a92fb7956d0d29a8727465e32d62c.jpg",
    ...
    local AvatarRow = Create("Frame", {
        Size = UDim2.new(1, 0, 0, self._mobile and 90 or 100),
        BackgroundTransparency = 1,
        LayoutOrder = 2,
        ZIndex = 4,
        Parent = AuthorContainer,
    ...
    AuthorTab:AddSeparator("CREDITS")
    AuthorTab:AddLabel("Roblox: xXHaNdEROXx")
    AuthorTab:AddLabel("Author: @inlawry")
    AuthorTab:AddLabel("Thanks for using!")
